...
import { WidgetProps, WidgetState, ChallengeResponse, SolutionResponse } from './types';
...
export const Widget: React.FC<WidgetProps> = ({
  ...
  return (
    <div 
      className={`wayn-widget wayn-widget--${color} wayn-widget--${state.state} ${className}`}
      style={style}
      onClick={handleClick}
      ...
      aria-live="polite"
      data-testid="wayn-widget"
    >
      <div className="wayn-widget__content">
        <div 
          className="wayn-widget__checkbox"
          style={state.state === 'verifying' ? { '--progress': `${state.progress}%` } as React.CSSProperties : undefined}
        />
        <span className="wayn-widget__text">
          {getDisplayText()}
        </span>
      </div>
      
      <a 
        href="https://github.com/islemci/wayn"
        className="wayn-widget__credits"
        target="_blank"
        rel="noopener noreferrer"
        onClick={(e) => e.stopPropagation()}
        aria-label="Secured by Wayn"
      >
        <span className="wayn-widget__credits-text">Secured by&nbsp;</span>
        Wayn
      </a>
